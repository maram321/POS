...
import { getSales } from '../services/salesListService';
import { saveSaleBill } from '../services/salesBillService';
...
import "./bill.css"
 ...
 class SalesBill extends Component {
    ...
    handleAddItem =(sale,quantity) => {
        const products =[...this.state.products]; 
        if (!products.some(e => e.sale === sale)) {
            products.push({
                "sale":sale,
                "quantity":quantity
            });
    ...
    calculateSubTotal = () => {
        ...
        for (let item of this.state.products)
            sum+= (item.sale.price*item.quantity)
        return sum;
    }
    ...
    render() { 
        ...
        <div  className='requires-no-scroll' style={{height:610}}>
            ...
            <div className='items-container'>
                {this.state.products.map((item,index) => 
                    <div className='item' key={index}>
                        <div className='img'>
                            <img src={"https://pos-node-up1x.onrender.com/images/"+item.sale.image} className="img-top" alt="not-found" />
                        </div>
                        <div className='information'>
                            <div className='information-name'>
                                <p className='name'>{item.sale.itemName}</p>
                            </div>
                            <div className='details'>
                                ...
                                <div className='button'>
                                    ...
                                    <h6>{item.quantity}</h6>
                                    <a href="#" className="btn btn-secondary" onClick={()=>this.handleDecrement(item)}><FaMinus style={{fontSize:22, marginInline:-8}}/></a>                                    
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div className='bill-top' style={{display:"flex", marginTop: -30}}>
                <p>قائمة الأصناف</p>
                <SearchBox className="searchbox" value={this.state.searchQuery} onChange={this.handleSearch} /> 
            </div>
            ...
            <div className='bill-list-container'>
                { sales.map(sale =>
                <div className='bill-item-container' key={sale._id}>
                    ...
                    <div className='item-info'>
                        ...
                        <div className='buttons'>
                            <a href="#" className="btn btn-dark" onClick={()=>this.handleAddItem(sale,1)}><IoMdAdd style={{fontSize:22, marginInline:-4, marginBottom:1}}/></a>
                        </div>
                    </div>
                </div>)}  
            </div>
